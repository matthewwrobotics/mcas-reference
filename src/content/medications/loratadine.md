---
name: Loratadine
aliases: ["Claritin", "loratadine"]
summary: Second-generation H1 antihistamine sold without a prescription; metabolised to desloratadine.
mechanismClass: H1 antihistamine
classRepresentative: cetirizine
mastCellBasis: downstream
studyDesigns:
  - randomised-controlled
establishedFor:
  - condition: "hay fever and upper respiratory allergy symptoms"
    basis: approved-us
regulatory: otc
evidenceLimits: >
  No trial cited here compared loratadine with the rest of its class in a mast
  cell condition. The class evidence was gathered with cetirizine. That
  loratadine is metabolised to another marketed antihistamine is a
  pharmacological relationship, not a result.
practicalNote:
  text: >
    The FAA lets pilots fly while taking it: loratadine is one of two oral
    antihistamines on its acceptable, non-sedating list. That is a regulator judging
    impairment risk across a population, not a promise about how any one person will
    feel.
  sourceUrl: "https://www.faa.gov/ame_guide/media/AllergyAntihistamineImmunotherapyMedication.pdf"
citations:
  - title: "FAA Guide for Aviation Medical Examiners: Allergy — Antihistamine & Immunotherapy Medication"
    url: "https://www.faa.gov/ame_guide/media/AllergyAntihistamineImmunotherapyMedication.pdf"
    sourceType: reference
    year: 2022
    accessed: 2026-08-25
  - title: "Loratadine US over-the-counter Drug Facts label"
    url: "https://dailymed.nlm.nih.gov/dailymed/drugInfo.cfm?setid=9e581ae1-d1e6-4038-86e8-fd0c5ab9878a"
    sourceType: drug-label
    accessed: 2026-08-25
  - title: "Cetirizine versus hydroxyzine and placebo in chronic idiopathic urticaria"
    url: "https://pubmed.ncbi.nlm.nih.gov/8893110/"
    sourceType: peer-reviewed
    year: 1996
    pmid: "8893110"
    accessed: 2026-08-21
  - title: "Mast Cell Activation Syndrome and Mastocytosis: Initial Treatment Options and Long-Term Management"
    url: "https://pubmed.ncbi.nlm.nih.gov/30961835/"
    sourceType: review
    year: 2019
    pmid: "30961835"
    accessed: 2026-08-20
lastVerified: 2026-08-25
---

## Mechanism of action

Loratadine is a second-generation H1 receptor antagonist. It blocks the H1
receptor after mast-cell mediators have been released, and penetrates the
central nervous system less than first-generation agents.

It is metabolised to desloratadine, which is separately marketed and has its own
entry. That relationship is why the two behave similarly, and why a reader who
has tried one may find the other described in the same terms.
