---
name: Cetirizine
aliases: ["Zyrtec"]
summary: Second-generation H1 antihistamine; blocks the H1 receptor and can still cause drowsiness despite a lower-sedation class profile.
mechanismClass: H1 antihistamine
classMembers:
  # Names without a page of their own. The four US-marketed members have pages
  # and are linked from this entry's card; bepotastine was listed here in error
  # — it is marketed in the US only as an ophthalmic solution, not an oral
  # antihistamine, so it belongs with the eye products.
  - "Bilastine (not marketed in the US)"
mastCellBasis: downstream
studyDesigns:
  - randomised-controlled
treatmentStep: 1
stepOrder: 1
establishedFor:
  - condition: "hay fever and upper respiratory allergy symptoms"
    basis: approved-us
  - condition: "chronic spontaneous urticaria"
    basis: randomised-trials
evidenceLimits: >
  The conventional first-step position rests on mechanism and consensus rather than
  on trials in this population. The randomised results concern chronic urticaria,
  and the current consumer label warns that drowsiness may occur; neither fact
  establishes an MCAS outcome or predicts sedation for an individual.
regulatory: otc
citations:
  - title: "Cetirizine versus hydroxyzine and placebo in chronic idiopathic urticaria"
    url: "https://pubmed.ncbi.nlm.nih.gov/8893110/"
    sourceType: peer-reviewed
    year: 1996
    pmid: "8893110"
    accessed: 2026-08-21
  - title: "Cetirizine hydrochloride US over-the-counter Drug Facts label"
    url: "https://dailymed.nlm.nih.gov/dailymed/drugInfo.cfm?setid=678dcc09-55da-4d9d-a39a-c450c61c387b"
    sourceType: drug-label
    accessed: 2026-08-21
  - title: "Mast Cell Activation Syndrome and Mastocytosis: Initial Treatment Options and Long-Term Management"
    url: "https://pubmed.ncbi.nlm.nih.gov/30961835/"
    sourceType: review
    year: 2019
    pmid: "30961835"
    accessed: 2026-08-20
  - title: "Pharmacotherapy of chronic spontaneous urticaria"
    url: "https://pubmed.ncbi.nlm.nih.gov/24195758/"
    sourceType: review
    year: 2013
    pmid: "24195758"
    accessed: 2026-08-20
lastVerified: 2026-08-21
---

Cetirizine occupies the histamine H1 receptor, preventing histamine already released from mast cells from producing its downstream effects — vasodilation, increased vascular permeability, pruritus, and sensory nerve stimulation. It acts on the mediator's target rather than on the cell, so it does nothing to reduce degranulation itself.

Its randomised evidence comes from chronic spontaneous urticaria and allergic rhinitis rather than from MCAS. H1 blockade is the first step in the stepwise mediator-blockade approach described in the mast cell disease management literature.
