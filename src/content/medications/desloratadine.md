---
name: Desloratadine
aliases: ["Clarinex", "desloratadine"]
summary: Second-generation H1 antihistamine; the active metabolite of loratadine, and the one member of this class that requires a prescription.
mechanismClass: H1 antihistamine
classRepresentative: cetirizine
mastCellBasis: downstream
studyDesigns:
  - randomised-controlled
establishedFor:
  - condition: "allergic rhinitis and chronic idiopathic urticaria"
    basis: approved-us
regulatory: approved-us-other
evidenceLimits: >
  The class evidence cited on this site was gathered with cetirizine, and none
  of it was in MCAS. Requiring a prescription is a regulatory fact about how
  this drug is sold, not a sign that it is stronger or better studied than the
  over-the-counter members of the same class.
practicalNote:
  text: >
    Like loratadine, it sits on the FAA's acceptable non-sedating list for pilots —
    a regulator's view of impairment risk across a population, not a promise about
    any individual. It is the prescription-only member of that pair.
  sourceUrl: "https://www.faa.gov/ame_guide/media/AllergyAntihistamineImmunotherapyMedication.pdf"
citations:
  - title: "FAA Guide for Aviation Medical Examiners: Allergy — Antihistamine & Immunotherapy Medication"
    url: "https://www.faa.gov/ame_guide/media/AllergyAntihistamineImmunotherapyMedication.pdf"
    sourceType: reference
    year: 2022
    accessed: 2026-08-25
  - title: "Desloratadine US prescription label"
    url: "https://dailymed.nlm.nih.gov/dailymed/drugInfo.cfm?setid=2a90b899-7746-43dc-ac8a-e754428eb30c"
    sourceType: drug-label
    accessed: 2026-08-25
  - title: "Cetirizine versus hydroxyzine and placebo in chronic idiopathic urticaria"
    url: "https://pubmed.ncbi.nlm.nih.gov/8893110/"
    sourceType: peer-reviewed
    year: 1996
    pmid: "8893110"
    accessed: 2026-08-21
  - title: "Mast Cell Activation Syndrome and Mastocytosis: Initial Treatment Options and Long-Term Management"
    url: "https://pubmed.ncbi.nlm.nih.gov/30961835/"
    sourceType: review
    year: 2019
    pmid: "30961835"
    accessed: 2026-08-20
lastVerified: 2026-08-25
---

## Mechanism of action

Desloratadine is the active metabolite of loratadine and a second-generation H1
receptor antagonist. It blocks the H1 receptor downstream of mediator release.

It is the only member of this class listed here that is dispensed on
prescription in the United States rather than over the counter. That difference
is regulatory: the others reached over-the-counter status through the FDA's
switch process, and this one has not. Nothing in the evidence cited here
distinguishes it clinically from the rest of the class.
